...
const navigate = useNavigate()
  ...
  return (
    <div className='flex justify-between items-center shadow-lg z-10 relative px-8 '>
      ...
      {!userInfo ? (
            <Link to={"/signin"}><Button isPrimary={true} text='Sign In Now' /></Link>

            
        ) : <div className='flex gap-4 items-center'><p>{userInfo.name}</p> <Button onPress={logoutHandler} isPrimary={false} text='Logout' /></div> }
    </div>
  )
}
